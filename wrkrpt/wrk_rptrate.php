    ...
    if (isset($_POST['Submit'])){ 
     if ($_POST['Submit'] == "Print") {
		...
		$shardSize = 2000;
	 	$sqliq = "";		   			
	 	$k = 0;
		$sql  = "SELECT distinct x.workid, x.prod_jobid, x.prod_jobrate, y.payratecd";
		$sql .= " FROM sew_barcode x, wor_detmas y";
    	$sql .= " where y.payratecd between '$fwid' and '$twid'";
		...
		while ($row = mysql_fetch_assoc($rs_result)) { 
	    	...
	    	$sqlc .= " from jobfile_master";
		   	...
		   	$sqlc  = "select workname, deptcd, deptde";
	    	$sqlc .= " from wor_detmas";
       		$sqlc .= " where workid ='$workid'";
       		$sql_resultc = mysql_query($sqlc);
       		$rowc = mysql_fetch_array($sql_resultc);
       		$wrknm  = mysql_real_escape_string($rowc['workname']);
       		$deptcd = $rowc['deptcd'];
       		$deptde = mysql_real_escape_string($rowc['deptde']);
		 ...
		 mysql_query($sqliq) or die("Unable Save In Temp Table ".mysql_error());
		mysql_close($db_link); 
     	 $fname = "wrkrpt02.rptdesign&__title=myReport"; 
         $dest = "http://".$var_prtserver.":8080/birt-viewer/frameset?__report=".$fname."&usernm=".$var_loginid."&dbsel=".$varrpturldb."&fprd=".$fwid."&tprd=".$twid;
         $dest .= urlencode(realpath($fname));

         //header("Location: $dest" );
         echo "<script language=\"javascript\">window.open('".$dest."','name','height=1000,width=1000,left=200,top=200');</script>";
         $backloc = "../wrkrpt/wrk_rptrate.php?menucd=".$var_menucode;
       	 echo "<script>";
...
function chkSubmit()
{
	...
	if (x==null || x=="")
	{
		alert("From Worker Payrate Cannot Be Blank");
		document.InpRawOpen.selfpr.focus();
		return false;
	}
	...
	if (x==null || x=="")
	{
		alert("To Worker Payrate Cannot Be Blank");
...
<body onload="setup()">
   <?php include("../topbarm.php"); ?> 
	...
	<fieldset name="Group1" style=" width: 758px; height: 205px;" class="style2">
	 <legend class="title">WORKER REPORT BY PAYRATE</legend>
	  <br />
	  <form name="InpRawOpen" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" style="height: 139px; width: 750px;" onSubmit= "return chkSubmit()">
		<table style="width: 807px; height: 102px;">
		    <tr>
		    	<td></td>
		    	<td>From Worker Payrate</td>
		    	<td>:</td>
		    	<td>
		    		<select name="selfpr" id ="selfpr" style="width: 150px">
			   		<?php
		    	...
		    	<td></td>
		    	<td>To Worker Payrate</td>
		    	<td>:</td>
		    	<td>
		    		<select name="seltpr" id ="seltpr" style="width: 150px">
			   		<?php
                   		$sql = "select paycode, paydesc from wor_payrate ORDER BY paycode";
                   		$sql_result = mysql_query($sql);
                   		echo "<option size =30 selected></option>";
