    ...
    if ($_POST['Submit'] == "Save") {
		...
		if ($sendto <> "") {
			...
			$sysno = '';
     		...
     		{
     			$sysno = '0';
     					$sysno_sql = "INSERT INTO system_number values ('RETURN', '$sysno')";
     		...
     		$issue_sysno = "RTN".$issue_sysno;


         	$vartoday = date("Y-m-d H:i:s");
				$sql = "INSERT INTO rawmat_return values 
					...
					foreach($_POST['procomat'] as $row=>$matcd ) {
						...
						if ($matcode <> "")
						{
							...
							$sql = "INSERT INTO rawmat_tran values 
						    		('RTN', '$issue_sysno', '$refno','$labelno', '$prorevdte', '$matcode', '0', '$matdesc', '0', '$issueqty','$var_loginid', '$vartoday','$var_loginid', '$vartoday')";
				...
				$updsysno_sql = "UPDATE system_number SET sysno = '$newsysno' WHERE type = 'RETURN'";
...
function validateForm()
{
	...
	if (x==null || x=="")
	{
	alert("Label No Must Not Be Blank");
	...
	for (var j = 1; j < rowCount; j++){
		...
		if (parseFloat(rowItem2) == 0 ){			
			alert ("Return Qty Cannot Be ZERO For Item : " +rowItem);		   
		    return false;
		}	       
...
<body onload="setup()">
  ...
  <div class="contentc">
	...
	<fieldset name="Group1" style=" width: 857px;" class="style2">
	 <legend class="title">RAWMAT RETURN</legend>
	  <br>	 
	  ...
	  <form name="InpJobFMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" onsubmit="return validateForm()">
		...
		<table style="width: 833px; height: 122px;">
		  ...
		  <table id="itemsTable" class="general-table" style="width: 841px">
              ...
              <th class="tabheader" style="width: 72px">Return Qty</th>
             </tr>
            </thead>
            <tbody>
