    ...
    if (isset($_POST['Submit'])){ 
     if ($_POST['Submit'] == "Update") {
        ...
        $selfdte = date("Y-m-d", strtotime($_POST['rptofdte']));
     	...
     	$sql = "SELECT x.rm_opening_id, x.openingdate, y.item_code, y.open_bas_cost, y.description ";
		$sql .= " FROM rawmat_opening x, rawmat_opening_tran y";
		$sql .= " Where x.openingdate between '$selfdte' and '$seltdte'";
		$sql .= " and x.rm_opening_id = y.rm_opening_id";
	//$sql .= " and x.rm_opening_id = 'OPB000247'";
	//$sql .= " and y.item_code = 'AN002600118'";	
   		$sql .= " ORDER BY x.rm_opening_id, y.item_code";  
		$rs_result = mysql_query($sql) or die("Cant Query Opening Table ".mysql_error()); 
		   			
		while ($row = mysql_fetch_assoc($rs_result)){
			
			$openid   = $row['rm_opening_id'];
			$opendte  = $row['openingdate'];
			$itemcode = mysql_real_escape_string($row['item_code']); 
			$opencost = $row['open_bas_cost'];
			$itmdesc  = mysql_real_escape_string($row['description']);
			...
			if ($itemcode != ""){
				...
				// 	$exhmth = date("n",strtotime($opendte)); 
				// 	$exhyr  = date("Y",strtotime($opendte));
				...
				//	$basecst = $opencost * $brate;
				#-------------------------------------------------------------------------------------------------
				
				#-------------Begin Update To Opening Tran table in based Currecnt Field--------------------------
				//	if ($opencost == ""){$opencost = 0;}
				//	if ($basecst == ""){$basecst = 0;}
				//	$sql5  = "Update rawmat_opening_tran set opening_cost  ='$opencost', ";
				...
				$sql7  = "Update rawmat_subcode set  cost_price ='".$opencost."'";
         		$sql7 .= " WHERE rm_code = '$itemcode' and cost_price = 0";
