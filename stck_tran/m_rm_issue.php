<?php
	 ...
	 if ($_POST['Submit'] == "Delete") {
     	if(!empty($_POST['procd']) && is_array($_POST['procd'])) 
     	{
           foreach($_POST['procd'] as $key) {
		     ...
		     $sql .= "WHERE rm_receive_id ='".$var_proccd."' and tran_type = 'ISS'";  
		 	 mysql_query($sql); 

 

		   }
		   $backloc = "../stck_tran/m_rm_issue.php?stat=1&menucd=".$var_menucode;
           echo "<script>";
	...
	if (isset($_POST['Submit'])){ 
     if ($_POST['Submit'] == "Print") {
     	 ...
     	 $backloc = "../stck_tran/inq_raw_matall.php?menucd=".$var_menucode."&t=I";
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>";   
     }
	  ...
	  <br>
        ...
        <form name="LstCatMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode;; ?>">
		 ...
		 <tr>
           ...
           <td style="width: 800px; height: 38px;" align="left">
           <?php
                ...
                if ($var_accadd == 0){
    	  	   ...
    	  	   $msgdel = "Are You Sure Delete Raw Mat Issue List?";
    	  	   include("../Setting/btndelete.php");
    	      ?></td>
		 </tr>
		 </table>
		 <br>
		 <table cellpadding="0" cellspacing="0" id="example" class="display" width="80%">
         <thead>
           <tr>
          <th></th>
          <th style="width: 26px">Issue No.</th>
          <th style="width: 43px">Ref No.</th>
          <th style="width: 63px">Label ID</th>
         ...
         <tr>
          ...
          <th class="tabheader" style="width: 26px">Issue No.</th>
           <th class="tabheader" style="width: 43px">Ref. No.</th>
          <th class="tabheader" style="width: 63px">Label ID</th>
          <th class="tabheader" style="width: 50px">Issue Date</th>
          <th class="tabheader" style="width: 18px">Detail</th>
          <th class="tabheader" style="width: 18px">Update</th>
		  <th class="tabheader" style="width: 18px">Delete</th>
         </tr>
         </thead>
		 <tbody>
		 <?php 
		    $sql = "SELECT rm_issue_id, refno, label_number, issuedate, upd_by, upd_on ";
			...
			while ($rowq = mysql_fetch_assoc($rs_result)) { 
				...
				$issuedate = date('d-m-Y', strtotime($rowq['issuedate']));
				$urlpop = 'upd_rm_issue.php';
				$urlvie = 'vm_rm_issue.php';
				echo '<tr bgcolor='.$defaultcolor.'>';
				
            	echo '<td>'.$numi.'</td>';
           		echo '<td>'.$rm_issue_id.'</td>';
           		echo '<td>'.$refno.'</td>';
            	echo '<td>'.$label_number.'</td>';
