					...
					foreach($_POST['procomat'] as $row=>$matcd ) {
						...
						if ($matcode <> "")
						{
							...
							$sql = "INSERT INTO rawmat_adj_tran values 
						    		('$adj_sysno', '$seqno', '', '$matcode', '$matdesc', '$matuom','$adjqty','$onhandbal', '$physicalqty', '$price')";
							mysql_query($sql) or die("Can't Insert Transaction ".mysql_error());
							
							
							$sql = "INSERT INTO rawmat_tran values 
						    		('ADJ', '$adj_sysno', '$refno','$remark', '$prorevdte', '$matcode', '$price', '$matdesc', '$onhandbal', '$adjqty ','$var_loginid', '$vartoday','$var_loginid', '$vartoday')";
...
<script  type="text/javascript" src="../stck_tran/jq-ac-script-adjust.js"></script>
...
function validateForm()
{
	...
	var x=document.forms["InpJobFMas"]["procomat1"].value;
	if (x==null || x=="")
	{
		alert("Please Enter Subcode No");
		document.getElementById('procomat1').focus();
		return false;
	}
	...
	for (var j = 1; j < rowCount; j++){
	   var idrowItem = "issueqtyid"+j;
       var rowItem = document.getElementById(idrowItem).value;
       
       var idrowI = "procomat"+j;
       var rowI = document.getElementById(idrowI).value;
       
	   if (rowI != ""){
	   	if (rowItem == ""){
	   		alert('Please Enter Adjust Physical Quantity!');
       ...
       if (rowItem != ""){
    	...
    	if(rowItem == 0.00) {
    	   alert('Adjust Physical Quantity Cannot Be Zero');
    	   document.getElementById(idrowItem).focus();
    	   return false;
    	}
       }	 
	 ...
	 <legend class="title">RAWMAT ADJUSTMENT</legend>
	  ...
	  <form name="InpJobFMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" onsubmit="return validateForm()">
		...
		<table style="width: 886px">
			...
			<input class="inputtxt" name="refno" id="refnoid" type="text" maxlength="10" style="width: 84px;" onchange ="upperCase(this.id)">
			</td>
		   ...
		   <img alt="Date Selection" src="../images/cal.gif" onclick="javascript:NewCssCal('prorevdte','ddMMyyyy')" style="cursor:pointer">
	  	  ...
	  	  <tr>
	  	    ...
	  	    <td style="width: 13px">:</td>
	  	    <td colspan="5">
			<input class="inputtxt" name="remark" id="remark" type="text" maxlength="100" style="width: 634px;" onchange ="upperCase(this.id)">
			</td>
	  	  </tr> 
		  ...
		  <table id="itemsTable" class="general-table" style="width: 841px">
            ...
            <tbody>
             ...
             <tr class="item-row">
                ...
                <td>
				<input name="procodesc[]" id="procodesc1" readonly="readonly" style="border:0; width: 303px;"></td>
                <td>
				<input name="procouom[]" id="procouom1" readonly="readonly" style="border:0; width: 48px;"></td>
                <td>
                <input name="procobal[]" id="procobal1" readonly="readonly"  style="width: 75px; border:0; text-align:right;"> </td>
				<td>
				<input name="issueqty[]" id="issueqtyid1" onBlur="calcCost('1');" style="width: 75px; text-align:right;"></td>  
             </tr>
            </tbody>
           </table>
