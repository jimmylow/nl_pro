<?php
    ...
    if ($_POST['Submit'] == "Update") {
		...
		if ($refno <> "") {
				...
				$vartoday = date("Y-m-d H:i:s");
				$sql = "update rawmat_opening set openingdate = '$prorevdte', refno ='$refno', remark='$remark', ";
				$sql .= "                       upd_by = '$var_loginid', upd_on='$vartoday' ";
				$sql .= "  where rm_opening_id = '$rm_opening_id'";
				...
				$sql .= "  where rm_receive_id ='$rm_opening_id' and tran_type = 'OPB'";	
					...
					foreach($_POST['procomat'] as $row=>$matcd ) {
						...
						if ($matcode <> "")
						{
							...
							$sql = "INSERT INTO rawmat_opening_tran values 
						    		('$rm_opening_id', '$seqno', '$matcode', '$matdesc', '$matuom','$openingqty','$qtyperpack','$openingcost','0')";
							mysql_query($sql) or die("Query 4: ".mysql_error());
							
							$sql = "INSERT INTO rawmat_tran values 
						    		('OPB', '$rm_opening_id', '$refno','$remark', '$prorevdte', '$matcode', '$openingcost', '$matdesc', '0', '$openingqty','$var_loginid', '$vartoday','$var_loginid', '$vartoday')";
			...
			$backloc = "../stck_tran/m_rm_opening.php?stat=4&menucd=".$var_menucode;
           	echo "<script>";
           	echo 'location.replace("'.$backloc.'")';
            echo "</script>"; 
		}
...
<script  type="text/javascript" src="../stck_tran/jq-ac-script-opening.js"></script>
...
<body onload="setup()">
  ...
  <?php
  	 $sql = "select * from rawmat_opening";
     $sql .= " where rm_opening_id ='".$rm_opening_id."'";
	 ...
	 <legend class="title">RAWMAT OPENING UPDATE :<?php echo $rm_opening_id;?></legend>
	  ...
	  <form name="InpJobFMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" onsubmit="return validateForm()">
		...
		<table style="width: 886px">
			...
			<input class="textnoentry1" name="rm_opening_id" id="prorevid" type="text" style="width: 84px;" readonly="readonly" tabindex="0" value="<?php echo $rm_opening_id; ?>">
			</td>
	  	  </tr> 
	  	  <tr>
	  	   <td></td>
		  ...
		  <table id="itemsTable" class="general-table" style = "width: 900px;">
            ...
            <tbody>
              <?php
             	$sql = "SELECT * FROM rawmat_opening_tran";
             	$sql .= " Where rm_opening_id='".$rm_opening_id ."'"; 
	    		$sql .= " ORDER BY seqno";  
