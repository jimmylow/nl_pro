<?php
    ...
    if ($_POST['Submit'] == "Update") {
		...
		if ($refno <> "") {
				...
				$sql .= "  where rm_trx_id ='$rm_receive_id' and tran_type = 'REC'";
					...
					foreach($_POST['procomat'] as $row=>$matcd ) {
						...
						if ($matcode <> "")
						{
							...
							$sql = "INSERT INTO rawmat_receive_tran values 
				    				('$rm_receive_id', '$seqno', '$refno', '$po_no', '$matcode', '$uprice', '$matdesc', '$matuom','$orderqty', '$qtyperpack', '$receiveqty')";
							mysql_query($sql);
...
function validateForm()
{
	...
	for (var j = 1; j < rowCount; j++){
        ...
        var idrowItem2 = "recqty"+j; // rec qty
