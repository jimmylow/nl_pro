<?php
	 ...
	 if ($_POST['Submit'] == "Delete") {
     	if(!empty($_POST['procd']) && is_array($_POST['procd'])) 
     	{
           foreach($_POST['procd'] as $key) {
		     ...
		     $sql = "DELETE FROM rawmat_return "; 
		     $sql .= "WHERE rm_return_id ='".$var_proccd."'";  
		 	 mysql_query($sql); 
		     ...
		     $sql .= "WHERE rm_receive_id ='".$var_proccd."' AND tran_type = 'RTN'";  
		 	 mysql_query($sql); 

		   }
		   $backloc = "../stck_tran/m_rm_return.php?stat=1&menucd=".$var_menucode;
           echo "<script>";
	...
	if (isset($_POST['Submit'])){ 
     if ($_POST['Submit'] == "Print") {
     	 ...
     	 $backloc = "../stck_tran/inq_raw_matall.php?menucd=".$var_menucode."&t=N";
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>";   
     }
...
<body>
  ...
  <div class="contentc">
	...
	<fieldset name="Group1" style=" width: 800px;" class="style2">
	 <legend class="title">RAW MAT RETURN LISTING</legend>
	  <br>
        ...
        <form name="LstCatMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode;; ?>">
		 ...
		 <tr>
           ...
           <?php
                $locatr = "rawmat_return.php?menucd=".$var_menucode;
                if ($var_accadd == 0){
    	  	   ...
    	  	   $msgdel = "Are You Sure Delete Selected Raw Mat return List?";
         ...
         <thead>
           <tr>
          <th></th>
          <th style="width: 78px">Return No.</th>
          <th>Ref No.</th>
          <th style="width: 74px">Issue No.</th>
          <th>Return Date</th>
          <th></th>
          <th></th>
		  <th></th>
         ...
         <tr>
          ...
          <th class="tabheader" style="width: 78px">Return No.</th>
           <th class="tabheader" style="width: 139px">Ref. No.</th>
          <th class="tabheader" style="width: 74px">Issue No.</th>
          <th class="tabheader" style="width: 106px">Return Date</th>
		 ...
		 <tbody>
		 <?php 
		    $sql = "SELECT rm_return_id, refno, issue_id, returndate, upd_by, upd_on ";
		    $sql .= " FROM rawmat_return ";
    		$sql .= " ORDER BY rm_return_id";   
			...
			while ($rowq = mysql_fetch_assoc($rs_result)) { 
				...
				$rm_return_id = htmlentities($rowq['rm_return_id']);
				$issue_id= htmlentities($rowq['issue_id']);

				$showdte = date('d-m-Y', strtotime($rowq['upd_on']));
				$returndate = date('d-m-Y', strtotime($rowq['returndate']));
				$urlpop = 'upd_rm_return.php';
				$urlvie = 'vm_rm_return.php';
				echo '<tr bgcolor='.$defaultcolor.'>';
				
            	echo '<td>'.$numi.'</td>';
           		echo '<td>'.$rm_return_id.'</td>';
           		echo '<td>'.$refno.'</td>';
            	echo '<td>'.$issue_id.'</td>';
            	...
            	if ($var_accvie == 0){
            		...
            		echo '<td align="center"><a href="'.$urlvie.'?rm_return_id='.$rm_return_id.'&menucd='.$var_menucode.'">[VIEW]</a>';'</td>';
            	}
	            if ($var_accupd == 0){
		            echo '<td align="center"><a href="#">[EDIT]</a>';'</td>';
	            }else{
		            echo '<td align="center"><a href="'.$urlpop.'?rm_return_id='.$rm_return_id.'&menucd='.$var_menucode.'">[EDIT]</a>';'</td>';
	            }
	            if ($var_accdel == 0){
