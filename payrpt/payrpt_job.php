    ...
    if (isset($_POST['Submit'])){ 
     if ($_POST['Submit'] == "Print") {
		...
		while ($row = mysql_fetch_assoc($rs_result)) { 
        	...
        	$sqliq .= "   '$barcodeno', '$workid', '$sewdate', '$qtydoz', '$qtypcs', '$var_loginid', '$wrknm')";
        	mysql_query($sqliq) or die("Unable Save In Temp Table ".mysql_error());
		 }

     	 $fname = "payrptjob.rptdesign&__title=myReport"; 
         $dest = "http://".$var_prtserver.":8080/birt-viewer/frameset?__report=".$fname."&usernm=".$var_loginid."&dbsel=".$varrpturldb."&fdte=".$fdte."&tdte=".$tdte."&fwid=".$fwid."&twid=".$twid."&fj=".$fjid."&tj=".$tjid;;
         $dest .= urlencode(realpath($fname));

         //header("Location: $dest" );
         echo "<script language=\"javascript\">window.open('".$dest."','name','height=1000,width=1000,left=200,top=200');</script>";
         $backloc = "../payrpt/payrpt_job.php?menucd=".$var_menucode;
       	 echo "<script>";
...
<body onload="setup()">
   <?php include("../topbarm.php"); ?> 
	...
	<fieldset name="Group1" style=" width: 758px; height: 205px;" class="style2">
	 <legend class="title">PRODUCTION PAYROLL REPORT BY JOB</legend>
	  <br />
