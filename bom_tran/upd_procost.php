<?php
    ...
    if ($_POST['Submit'] == "Update") {
		...
		if ($vmprodcd <> "") {
				...
				$sql = "Update prod_matmain Set docdate = '$vmrevdte', rmcost ='$vmmatcst', labcost='$vmlabcst', ";
				$sql .= "                       exftrycost = '$vmextamt', overcost ='$vmovecst', totmscb='$vmmixcst', ";
				$sql .= "                       salestper = '$vmsalper', salesamt ='$vmsalamt', totcost='$vmtotcst', ";
				$sql .= "                       totamt = '$vmtotamt', modified_by = '$var_loginid', modified_on='$vartoday ' ";
				$sql .= "  Where prod_code ='$vmprodcd'";
   	...
   	for (var j = 2; j < rowCount; j++){
        ...
        select: function(event, ui) {
            var $itemrow = $(this).closest('tr');
                    ...
                    var idp = this.id;
                    var lidp = idp.substring(8);  
                    var idprocomat   = "#procomat"+lidp;
	    			var idprocodesc  = "#procodesc"+lidp;
	    			var idprocouom   = "#procouom"+lidp;
	    			var idprocomark  = "#procomark"+lidp;
	    			var idprocospre  = "#procospre"+lidp;
	    			var idprococut   = "#prococut"+lidp;
	    			var idprocobund  = "#procobund"+lidp;
	    			var idprocoucost = "#procoucost"+lidp;
                    
                    $itemrow.find(idprocomat).val(ui.item.rm_code);
                    $itemrow.find(idprocodesc).val(ui.item.remark);
                    $itemrow.find(idprocouom).val(ui.item.uom);
                    $itemrow.find(idprocomark).val(ui.item.mark);
                    $itemrow.find(idprocospre).val(ui.item.spread);
                    $itemrow.find(idprococut).val(ui.item.cut);
                    $itemrow.find(idprocobund).val(ui.item.bundle);
 ...
 <!-- <?php include("../sidebarm.php"); ?>-->
  ...
  <div class ="contentc">
	...
	<fieldset name="Group1" style=" width: 1150px;" class="style2">
	 <legend class="title">PRODUCT COSTING UPDATE <?php echo $var_prodcd;?></legend>
	  <br>	 
	  ...
	  <form name="InpJobFMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" onsubmit="return validateForm()">
		...
		<table style="width: 886px">
		   ...
		   <td>:</td>
		   <td>
		   <input class="inputtxt" name="prorevdte" id ="prorevdte" type="text" style="width: 128px;" value="<?php  echo $docdte; ?>" tabindex="1" >
		   <img alt="Date Selection" src="../images/cal.gif" onclick="javascript:NewCssCal('prorevdte','ddMMyyyy')" style="cursor:pointer; height: 16px;"></td>
	  	  </tr>
	  	   <tr>
	  	   <td></td>
		  ...
		  <tr>
		   ...
		   <td>:</td>
		   <td>
		   <input name="totalexft" id ="totalexft0" type="text" style="width: 156px; text-align:center;" tabindex="4" onblur="showexftrDecimal(this.value)" value="<?php echo $extamt; ?>"></td>
		  </tr> 
		  <tr>
	  	   <td style="height: 22px"></td>
	  	   <td style="width: 126px; height: 22px;">Labour</td>
	  	   <td style="width: 13px; height: 22px;">:</td>
	  	   <td style="width: 239px; height: 22px;">
		   <input name="totallabc" id ="totallabcid" type="text" style="width: 156px; text-align:center;"  tabindex="2" value="<?php echo $labcst; ?>" onblur="showlabDecimal(this.value)"></td>
		   <td style="height: 22px"></td>
		   <td style="width: 136px; height: 22px;">Sales Tax (%)</td>
		   <td style="height: 22px">:</td>
		   <td style="height: 22px">
		   <input name="totaltaxc" id ="totaltaxd" type="text" style="width: 156px; " class="textnoentry1" onblur="showtaxperDec(this.value)" value="<?php echo $saltper; ?>"></td>
	  	  </tr>
		  <tr>
	  	   <td></td>
	  	   <td>Overhead</td>
	  	   <td style="width: 13px">:</td>
	  	   <td style="width: 239px">
		   <input name="totaloveh" id ="totalovehid" type="text" style="width: 156px; text-align:center;" tabindex="3" onblur="showoveDecimal(this.value)" value="<?php echo $ovecst;?>">
		   </td>
		   <td></td>
		  ...
		  <table id="itemsTable" class="general-table">
              ...
              <th class="tabheader" style="width: 140px; height: 57px;">Raw Material Item</th>
              <th class="tabheader" style="width: 303px; height: 57px;">Description</th>
              <th class="tabheader" style="width: 48px; height: 57px;">UOM</th>
              <th class="tabheader" style="width: 75px; height: 57px;">Unit <br>Cost(RM)</th>
              <th class="tabheader" style="width: 75px; height: 57px;">Unit<br>Consumption</th>
              <th class="tabheader" style="width: 75px; height: 57px;">Cost</th>
              <th class="tabheader" style="width: 75px; height: 57px;">Mark</th>
              <th class="tabheader" style="width: 75px; height: 57px;">Spread</th>
			  <th class="tabheader" style="width: 75px; height: 57px;">Cut</th>
			  <th class="tabheader" style="width: 75px; height: 57px;">Bundle</th>
             </tr>
            </thead>
            <tbody>
              <?php
                ...
                if ($i == 1){
                	echo '<tr class="item-row">';	
					echo '<td><input name="seqno[]" id="seqno" value="'.$i.'" readonly="readonly" style="width: 27px; border:0;"></td>'; 
					echo '<td><input name="procomat[]" value="'.htmlentities($rowq['rm_code']).'" id="procomat'.$i.'" class="autosearch" style="width: 161px" onblur="get_desc(this.value, '.$i.')" onchange ="upperCase(this.id)"></td>';
                	echo '<td><input name="procodesc[]" value="'.$rowq['rm_desc'].'" id="procodesc'.$i.'" style="width: 303px;" onchange ="upperCase(this.id)"></td>';
             		echo '<td><input name="procouom[]" value="'.$rowq['rm_uom'].'" id="procouom1" readonly="readonly" style="border-style: none; border-color: inherit; border-width: 0; width: 48px;"></td>';
                    echo '<td><input name="procoucost[]" id="procoucost'.$i.'" value="'.$rowq['rm_ucost'].'"style="width: 75px; text-align:right;" onBlur="calcCost('.$i.');"></td>';             	
                	echo '<td><input name="prococompt[]" value="'.$rowq['rm_comps'].'" id="prococompt'.$i.'" style="width: 75px; text-align:right;" onblur="checkcompsum('.$i.')" onkeyup="calcCost('.$i.');" ></td>';
                	echo '<td><input name="prococost[]" value="'.$rowq['rm_costing'].'" id="prococost'.$i.'" readonly="readonly" style="width: 75px; border:0; text-align:right;"></td>';
