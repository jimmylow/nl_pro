...
<body>
  ...
  <div class ="contentc">
	...
	<fieldset name="Group1" style=" width: 569px;" class="style2">
	 <legend class="title">PLANNING DETAIL - BY PRODUCT (3)</legend>
	  <br>	 
	  <form name="InpMDETMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" onsubmit="return validateForm()" style="width: 553px">
	    ...
	    <br>
		  ...
		  <table id="itemsTable" class="general-table" style="width: 548px">
          	<thead>
          	 <tr>
              <th class="tabheader" style="width: 41px">#</th>
              <th class="tabheader" style="width: 81px">Color Code</th>
              <th class="tabheader">Color Description</th>
              <th class="tabheader" style="width: 76px">Order Qty</th> 
              <th class="tabheader" style="width: 85px">Plan Detail</th>
             </tr>
            </thead>
            <tbody>
       		<?php 
       		  $sql1  = "SELECT distinct procdcol ";
			  ...
			  while ($row1 = mysql_fetch_assoc($rs_result1)){
			  	$prodcdcol = htmlentities($row1['procdcol']);
			  	
			  	$sqlcd = "select clr_desc from pro_clr_master  ";
        		$sqlcd .= " where clr_code ='$prodcdcol'";
        		...
        		$sqlcd .= " where procdcol ='$prodcdcol' and usernm = '$var_loginid'";
        		$sql_resultcd = mysql_query($sqlcd) or die("Can't query temp Table 1:".mysql_error());
        		$rowcd = mysql_fetch_array($sql_resultcd);
        		$ordqty = $rowcd[0];

                $urlviwcol = "plancol_upd.php?col=".$prodcdcol;
				echo '<tr class="item-row">';
             	echo '<td><input name="seqno[]" id="seqno" value="'.$i.'" readonly="readonly" style="width: 41px; border:0;"></td>';
             	echo '<td><input name="plancol[]" value="'.$prodcdcol.'" id="plcolcode'.$i.'" readonly="readonly" style="width: 81px"></td>';
             	echo '<td><input name="plandes[]" value="'.$coldesc.'" id="plcoldesc'.$i.'" readonly="readonly" style="border-style: none;"></td>';
             	echo '<td><input name="planqty[]" value="'.$ordqty.'" id="plcolqty'.$i.'" readonly="readonly" style="border-style: none; width: 76px; text-align:right;"></td>';
