<?php
    ...
    if ($_POST['Submit'] == "Save") {
     	...
     	if ($decd <> "") {
     		$vartoday = date("Y-m-d");       
        	$sql = "INSERT INTO wor_country values 
            	   ('$decd', '$dede', '$var_loginid', '$vartoday', '$var_loginid', '$vartoday')";
     	 	mysql_query($sql) or die("Unable To Save country Code ".mysql_error()); 
			mysql_close($db_link); 
     	 	$backloc = "../workefile/wo_country_mas.php?menucd=".$var_menucode;
         	echo "<script>";
    ...
    if ($_POST['Submit'] == "Delete") {
     if(!empty($_POST['paycd']) && is_array($_POST['paycd'])) 
     {
           ...
           foreach($_POST['paycd'] as $value ) {
		    	$sql = "DELETE FROM wor_country WHERE countrycode ='$value'"; 
		 		mysql_query($sql); 
		   }
		   mysql_close($db_link); 
		   $backloc = "../workefile/wo_country_mas.php?menucd=".$var_menucode;
    ...
    if (isset($_POST['Submit'])){ 
     if ($_POST['Submit'] == "Print") {
		...
		// Redirect browser
        $fname = "wo_country.rptdesign&__title=myReport"; 
...
function AjaxFunction(x){
   ...
   if (x != ""){	
   	var strURL="aja_chk_country.php?de="+x;
   	var req = getXMLHTTP();
		    ...
		    if (req.status == 200)
   	        {    
   	        	if (req.responseText == 1){    	
					document.getElementById("msgcd").innerHTML = "<font color='red'>This country Code Has Been Create</font>";
		    	}else{
...
function chkSubmit () {
	...
	if (varchk == 0){
		alert("This country Code Has Been Create");
		document.InpPayMas.paycd.focus();
		return false;
	}	
...
<div class="contentc">
	<fieldset name="Group1" style=" width: 692px;" class="style2">
	 <legend class="title">WORKER COUNTRY MASTER</legend>
	  <br>
	  ...
	  <form name="InpPayMas" method="POST" onSubmit= "return chkSubmit()" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" style="height: 134px; width: 641px;">
		<table>
	  	...
	  	<?php
				$sql = "SELECT countrycode, countrydesc ";
				$sql .= " FROM wor_country";
    		    $sql .= " ORDER BY countrycode";  
        ...
        <form name="LstColMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>">
         ...
         <thead>
         	<tr>
          	...
          	<th style="width: 91px">Country Code</th>
         	...
         	<tr>
          	...
          	<th class="tabheader" style="width: 91px">Country Code</th>
		 ...
		 <?php 
			...
			while ($row = mysql_fetch_assoc($rs_result)) {
				$pacode = htmlentities($row['countrycode']);
				$padesc = htmlentities($row['countrydesc']); 
 
				$urlpop = 'upd_countrymas.php?cd='.$pacode."&de=".$padesc.'&menucd='.$var_menucode;
				echo '<tr>';
            	...
            	if ($var_accupd == 0){
            		echo '<td align="center"><a href="#" title="You Are Not Authorice To Update The country Code">[EDIT]</a>';'</td>';
            	}else{
            		echo '<td align="center"><a href="'.$urlpop.'">[EDIT]</a></td>';
            	}
            
            	if ($var_accdel == 0){
            		echo '<td align="center"><input type="checkbox" DISABLED  name="paycd[]" value="'.$pacode.'" title="You Are Not Authorice To Update The country Code"/>'.'</td>';
            	}else{
