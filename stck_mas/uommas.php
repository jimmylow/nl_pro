<?php
    ...
    if ($_POST['Submit'] == "Save") {
     ...
     if ($uomcd <> "") {
      ...
      $query_id = mysql_query($var_sql) or die ("Cant Check UOM Code");
      $res_id = mysql_fetch_object($query_id);

      if ($res_id->cnt > 0 ) {
	     $backloc = "../stck_mas/uommas.php?stat=3&menucd=".$var_menucode;
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>";       
      }else {
       
         $sql = "INSERT INTO uom_master values 
                ('$uomcd', '$uomdesc','$var_loginid', CURDATE(), '$var_loginid', CURDATE(),'$uompck')";
                           
     	 mysql_query($sql); 
     	 $backloc = "../stck_mas/uommas.php?stat=1&menucd=".$var_menucode;
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>";       } 
     }else{
       $backloc = "../stck_mas/uommas.php?stat=4&menucd=".$var_menucode;
       echo "<script>";
       echo 'location.replace("'.$backloc.'")';
       echo "</script>";      
     }
    ...
    if ($_POST['Submit'] == "Delete") {
     if(!empty($_POST['uomcd']) && is_array($_POST['uomcd'])) 
     {
           ...
           foreach($_POST['uomcd'] as $value ) {
		    $sql = "DELETE FROM uom_master WHERE uom_code ='".$value."'"; 
		 	mysql_query($sql); 
		   }
		   $backloc = "../stck_mas/uommas.php?stat=1&menucd=".$var_menucode;
           echo "<script>";
...
<body onload="document.InpColMas.uomcd.focus()">
  ...
  <div class="contentc">
	<fieldset name="Group1" style="width: 758px;" class="style2">
	 <legend class="title">STOCK UOM MASTER</legend>
	  <br>
	  <fieldset name="Group1" style="width: 713px; height: 168px">
	  <form name="InpColMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" style="height: 134px; width: 696px;">
	   ...
	   <br/><br/>
	  	<?php
				$sql = "SELECT uom_code, uom_desc, uom_pack, modified_by, modified_on ";
				$sql .= " FROM uom_master";
        ...
        <form name="LstColMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>">
		 <table>
		 <tr>
		   <td style="width: 740px; height: 38px;" align="right">
		       <?php
         ...
         <thead>
         	<tr>
         	 <th></th>
         	 <th style="width: 82px">UOM Code</th>
         	 <th>UOM Description</th>
         	 <th style="width: 104px">UOM Pack</th>
         	 <th></th>
         	 <th></th>
         	</tr>
         	<tr>
         	 <th class="tabheader" style="width: 27px">#</th>
         	 <th class="tabheader" style="width: 82px">UOM Code</th>
         	 <th class="tabheader" style="width: 303px">UOM Description</th>
         	 <th class="tabheader" style="width: 104px">UOM Pack</th>
		 ...
		 <?php 
            ...
            echo '<td><a href="'.$urlpop.'?uomcd='.$row['uom_code'].'&uomde='.$row['uom_desc'].'&uompck='.$row['uom_pack'].'&menucd='.$var_menucode.'">[EDIT]</a>';'</td>';
            }
            if ($var_accdel == 0){
