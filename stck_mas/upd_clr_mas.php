<?php
    ...
    if($var_loginid == "") { 
	  ...
	  $var_clrde = $_GET['clrde'];
	  $var_menucode = $_GET['menucd'];
	}
    
     if ($_POST['Submit'] == "Update") {
    
 		$var_clrcd  = $_POST['clrcdu'];
        $clrdescd = $_POST['clrdeu'];
        $var_menucode  = $_POST['menudcode'];

         if ($var_clrcd <> "") {
        
         $vartoday = date("Y-m-d H:i:s");
         $sql = "Update colour_master set colour_desc ='$clrdescd',";
         $sql .= " modified_by='$var_loginid',";
         $sql .= " modified_on='$vartoday' WHERE colour_code = '$var_clrcd'";
           
       	 mysql_query($sql); 
        
         $backloc = "../stck_mas/colormas.php?menucd=".$var_menucode;
         echo "<script>";
...
<?php
 	$sql = "select create_by, 	creation_time, 	modified_by, 	modified_on";
   	...
   	$sql .= " where colour_code ='".$var_clrcd ."'";   
...
<div class="contentc">
	...
	<fieldset name="Group1" style="height: 238px; width: 718px;" class="style2">
	 <legend class="title">EDIT COLOR CODE MASTER</legend>
	 <br>
	  <form name="InpColMas" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" style="height: 134px">
		...
		<table>
	  	  <tr>
	  	    ...
	  	    <td style="width: 147px" class="tdlabel">Color Code</td>
	  	    <td>:</td>
	  	    <td>
	  	    <?php
	  	       if (isset($var_clrcd)){
	  	  ...
	  	  <tr>
	  	    ...
	  	    <td style="width: 147px" class="tdlabel">Color Description</td>
	  	    <td>:</td>
	  	    <td>
	  	     <?php
	  	       if (isset($var_clrde)){
