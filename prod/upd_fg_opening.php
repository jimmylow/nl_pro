<?php
    ...
    if($var_loginid == "") { 
      ...
      $fg_opening_id = $_GET['fg_opening_id'];
      include("../Setting/ChqAuth.php");
    }
    
    if ($_POST['Submit'] == "Update") {
		$fg_opening_id = $_POST['fg_opening_id'];
		...
		if ($refno <> "") {
				...
				$vartoday = date("Y-m-d H:i:s");
				$sql = "update fg_opening set openingdate = '$prorevdte', refno ='$refno', remark='$remark', ";
				$sql .= "                       upd_by = '$var_loginid', upd_on='$vartoday' ";
				$sql .= "  where fg_opening_id = '$fg_opening_id'";
				mysql_query($sql) or die("Query 1: ".mysql_error());
			
				// to delete from fg opening details table
				$sql =  "delete from fg_opening_tran";
				$sql .= "  where fg_opening_id ='$fg_opening_id'";
				mysql_query($sql) or die("Query 2: ".mysql_error());
				
				// to delete from fg history table
				$sql =  "delete from fg_tran";  
				$sql .= "  where rm_receive_id ='$fg_opening_id' and tran_type = 'OPB'";	
				...
				{	
					foreach($_POST['procomat'] as $row=>$matcd ) {
						...
						if ($matcode <> "")
						{
							...
							$sql = "INSERT INTO fg_opening_tran values 
						    		('$fg_opening_id', '$seqno', '$matcode', '$matdesc', '$matuom','$openingqty','$qtyperpack','$openingcost','0')";
							mysql_query($sql) or die("Query 4: ".mysql_error());
							
							$sql = "INSERT INTO fg_tran values 
						    		('OPB', '$fg_opening_id', '$refno','$remark', '$prorevdte', '$matcode', '$openingcost', '$matdesc', '0', '$openingqty','$var_loginid', '$vartoday','$var_loginid', '$vartoday')";
							mysql_query($sql) or die("Query 5: ".mysql_error());

           				}	
					}
				}
				$backloc = "../prod/m_fg_opening.php?menucd=".$var_menucode;
           		echo "<script>";
           		echo 'location.replace("'.$backloc.'")';
            	echo "</script>"; 		
		}else{
			$backloc = "../prod/m_fg_opening.php?stat=4&menucd=".$var_menucode;
           	echo "<script>";
           	echo 'location.replace("'.$backloc.'")';
            echo "</script>"; 
		}
...
<body onload="setup()">
  ...
  <div class="contentc">
	...
	<fieldset name="Group1" style=" width: 950px;" class="style2">
	 <legend class="title">FINISH GOODS OPENING UPDATE :<?php echo $fg_opening_id;?></legend>
	  <br>	 
	  ...
	  <form name="InpJobFMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" onsubmit="return validateForm()">
		...
		<table style="width: 886px">
			...
			<input class="textnoentry1" name="fg_opening_id" id="prorevid" type="text" style="width: 84px;" readonly="readonly" tabindex="0" value="<?php echo $fg_opening_id; ?>">
			</td>
	  	  </tr> 
	  	  <tr>
	  	   <td></td>
		  ...
		  <table id="itemsTable" class="general-table" style = "width: 900px;">
            ...
            <tbody>
              <?php
             	$sql = "SELECT * FROM fg_opening_tran";
             	$sql .= " Where fg_opening_id='".$fg_opening_id ."'"; 
	    		$sql .= " ORDER BY seqno";  
		 ...
		 <table>
		  	<tr>
				...
				<?php
				 $locatr = "m_fg_opening.php?menucd=".$var_menucode;
