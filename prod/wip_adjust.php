<?php
    ...
    if ($_POST['Submit'] == "Save") {
		...
		if ($refno <> "") {
     		...
     		{
     			$sysno = '0';
     					$sysno_sql = "INSERT INTO system_number values ('ADJUST_WIP', '$sysno')";
     		...
     		$adjust_sysno  = str_pad($newsysno , 6, '0', STR_PAD_LEFT);
     		$adjust_sysno = "ADJ".$adjust_sysno;


         	$vartoday = date("Y-m-d H:i:s");
				$sql = "INSERT INTO wip_adjust values 
						('$adjust_sysno', '$refno','$prorevdte', '$remark', 
					...
					foreach($_POST['procomat'] as $row=>$matcd ) {
						...
						if ($matcode <> "")
						{
							...
							$sql = "INSERT INTO wip_adjust_tran values 
						    		('$adjust_sysno', '$seqno', '$matcode', '$matdesc', '$matuom','$adjustqty','$qtyperpack', '$adjustcost', '0')";
							mysql_query($sql) or die("query 2 :".mysql_error());
							
							$sql = "INSERT INTO wip_tran values 
						    		('ADJ', '$adjust_sysno', '$refno','$remark', '$prorevdte', '$matcode', '$adjustcost', '$matdesc', '$qtyperpack', '$adjustqty','$var_loginid', '$vartoday','$var_loginid', '$vartoday')";
				...
				$updsysno_sql = "UPDATE system_number SET sysno = '$newsysno' WHERE type = 'ADJUST_WIP'";
...
<body onload="setup()">
  ...
  <div class="contentc">
	...
	<fieldset name="Group1" style=" width: 930px;" class="style2">
	 <legend class="title">WIP STOCK ADJUSTMENT</legend>
	  <br>	 
		  ...
		  <table id="itemsTable" class="general-table" style = "width: 900px;">
            ...
            <tbody>
             ...
             <tr class="item-row">
				...
				<td>
				<input name="adjustqty[]" id="adjustqtyid1" onBlur="calcCost('1');" style="width: 75px"></td>  
             </tr>
            </tbody>
           </table>
