<?php
	 ...
	 if ($_POST['Submit'] == "Delete") {
     	if(!empty($_POST['procd']) && is_array($_POST['procd'])) 
     	{
           foreach($_POST['procd'] as $key) {
		     ...
		     $sql = "DELETE FROM wip_opening "; 
		     $sql .= "WHERE wip_opening_id ='".$var_proccd."'";  
		 	 mysql_query($sql); 
		   ...
		   $backloc = "../prod/m_wip_opening_bar.php?stat=1&menucd=".$var_menucode;
           echo "<script>";
	...
	if (isset($_POST['Submit'])){ 
     if ($_POST['Submit'] == "Print") {
     	 ...
     	 $backloc = "../prod/inq_wip_open.php?menucd=".$var_menucode;
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>";   
     }
...
<body>
  ...
  <div class="auto-style1">
	...
	<fieldset name="Group1" style=" width: 800px;" class="style2">
	 <legend class="title">WIP OPENING LISTING xxx--TESTING---xxx</legend>
	  <br>
        ...
        <form name="LstCatMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode;; ?>">
		 ...
		 <tr>
           ...
           <?php
                $locatr = "wip_opening_bar.php?menucd=".$var_menucode;
                if ($var_accadd == 0){
				...
				//}
    	  	   ...
    	  	   $msgdel = "Are You Sure Delete WIP opening List?";
		 ...
		 <tbody>
		 <?php 
		    $sql = "SELECT distinct x.wip_opening_id, refno, openingdate, upd_by, upd_on ";
		    $sql .= " FROM wip_opening x, wip_opening_tran y ";
    		$sql .= " WHERE x.wip_opening_id = y.wip_opening_id";   
    		$sql .= " AND ticketno <> '' ";   
    		$sql .= " ORDER BY wip_opening_id";   
			...
			while ($rowq = mysql_fetch_assoc($rs_result)) { 
				...
				$wip_opening_id = htmlentities($rowq['wip_opening_id']);
				...
				$urlpop = 'upd_wip_opening_bar.php';
				$urlvie = 'vm_wip_opening_bar.php';
				echo '<tr bgcolor='.$defaultcolor.'>';
				
            	echo '<td>'.$numi.'</td>';
           		echo '<td>'.$wip_opening_id.'</td>';
           		echo '<td>'.$refno.'</td>';
            	...
            	if ($var_accvie == 0){
            		...
            		echo '<td align="center"><a href="'.$urlvie.'?wip_opening_id='.$wip_opening_id.'&menucd='.$var_menucode.'">[VIEW]</a>';'</td>';
            	}
	            if ($var_accupd == 0){
		            echo '<td align="center"><a href="#">[EDIT]</a>';'</td>';
	            }else{
		            echo '<td align="center"><a href="'.$urlpop.'?wip_opening_id='.$wip_opening_id.'&menucd='.$var_menucode.'">[EDIT]</a>';'</td>';
	            }
	            if ($var_accdel == 0){
