	 ...
	 if ($_POST['Submit'] == "Delete") {
     	if(!empty($_POST['procd']) && is_array($_POST['procd'])) 
     	{
           foreach($_POST['procd'] as $key) {
		     ...
		     $sql = "DELETE FROM fg_adjust "; 
		     $sql .= "WHERE fg_adjust_id ='".$var_proccd."'";  
		 	 mysql_query($sql); 
		   ...
		   $backloc = "../prod/m_fg_adjust.php?stat=1&menucd=".$var_menucode;
           echo "<script>";
...
<body>
  ...
  <div class="auto-style1">
	...
	<fieldset name="Group1" style=" width: 800px;" class="style2">
	 <legend class="title">FINISH GOODS ADJUSTMENT LISTING</legend>
	  <br>
        ...
        <form name="LstCatMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode;; ?>">
		 ...
		 <tr>
           ...
           <?php
                $locatr = "fg_adjust.php?menucd=".$var_menucode;
                if ($var_accadd == 0){
				...
				//}
    	  	   ...
    	  	   $msgdel = "Are You Sure Delete FG adjust List?";
		 ...
		 <tbody>
		 <?php 
		    $sql = "SELECT distinct fg_adjust_id, refno, adjustdate, upd_by, upd_on ";
		    $sql .= " FROM fg_adjust ";
    		$sql .= " ORDER BY fg_adjust_id";   
			...
			while ($rowq = mysql_fetch_assoc($rs_result)) { 
				...
				$fg_adjust_id = htmlentities($rowq['fg_adjust_id']);
				...
				$urlpop = 'upd_fg_adjust.php';
				$urlvie = 'vm_fg_adjust.php';
				echo '<tr bgcolor='.$defaultcolor.'>';
				
            	echo '<td>'.$numi.'</td>';
           		echo '<td>'.$fg_adjust_id.'</td>';
           		echo '<td>'.$refno.'</td>';
            	...
            	if ($var_accvie == 0){
            		...
            		echo '<td align="center"><a href="'.$urlvie.'?fg_adjust_id='.$fg_adjust_id.'&menucd='.$var_menucode.'">[VIEW]</a>';'</td>';
            	}
	            if ($var_accupd == 0){
		            echo '<td align="center"><a href="#">[EDIT]</a>';'</td>';
	            }else{
		            echo '<td align="center"><a href="'.$urlpop.'?fg_adjust_id='.$fg_adjust_id.'&menucd='.$var_menucode.'">[EDIT]</a>';'</td>';
	            }
	            if ($var_accdel == 0){
