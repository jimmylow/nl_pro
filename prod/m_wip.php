...
<?php 
	$sql = "DELETE FROM tmpwipstk ";
	$sql.= " WHERE username = '$var_loginid' ";
	mysql_query($sql) or die("Failed to DELETE Temp Table tmpwipstk : ".mysql_error());
	
	
	$shardSize = 3000;
 	$sqliq = "";		   			
 	$k = 0;
	$sql = "SELECT DISTINCT prod_code ";
	$sql .= " FROM pro_jobmodel";
	//$sql .= " WHERE prod_code in ('E8025/3-WT-120C')";
	...
	while ($rowq = mysql_fetch_assoc($rs_result)) {
	    ...
	    if ($k % $shardSize == 0) {
			...
			$sqliq = 'Insert Into tmpwipstk values';
		}
		$sqliq .= (($k % $shardSize == 0) ? '' : ', ') . "('".$rowq['prod_code']."', '$proddesc', '$onhandbal', '$unitprice', '$amt', '$var_loginid')";
		$k = $k + 1;
	}
	if ($sqliq <> ""){
		mysql_query($sqliq) or die("Unable Save In Temp Table ".mysql_error());
...
<body onload="document.InpJobFMas.prod_code.focus()">
 <?php include("../topbarm.php"); ?> 
  <div class="contentc">
	 ...
	 <legend class="title">Work In Progress Stock </legend>
	 <table style="width: 100%; height: 66px;">
	   <?php  
	   	$sql = "select sum(onhand), sum(onhand * uprice)  from tmpwipstk where username = '$var_loginid'";
	    $sql_result = mysql_query($sql);
	  ...
	  <form name="LstCatMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode;; ?>">
		 ...
		 <tbody>
		 <?php 
        	$sql = "select * from tmpwipstk where username = '$var_loginid'";
			//$sql .= " where onhand > 0 ";
			...
			while ($rowq = mysql_fetch_assoc($rs_result)) { 
        		...
        		$sql1 = "select prod_desc from pro_cd_master";
				$sql1 .= " where prod_code ='$productcode'";
				$sql_result1 = mysql_query($sql1);
				$row1 = mysql_fetch_array($sql_result1);
				$productdesc = htmlentities($row1[0]);

				$showdte = date('d-m-Y', strtotime($rowq['modified_on']));
				$urlvie = 'vm_wip_stock.php';
				echo '<tr>';
            	echo '<td>'.$numi.'</td>';
