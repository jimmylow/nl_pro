	...
	if ($ticketno<> "") {
			...
			/*$var_sql = " SELECT count(*) as cnt from sew_entry";
	      	...
	      	if ($res_id->cnt > 0 ){
	            ...
	            $sql2  = " UPDATE sew_entry   set buyerorder = '$buyerord',";
				$sql2 .= "                        buyer = '$sbuycd',  ";
				$sql2 .= "                        orderdate = '$sorddte', ";
				$sql2 .= "                        productiondate = '$prorevdte',";
				$sql2 .= "                        deliverydate = '$sexpddte', ";
				$sql2 .= "                        batchno = '$batchno'," ;
				$sql2 .= "                        productcode = '$selmaincode'," ;
				$sql2 .= "                        productqty = '$productqty'," ;
				$sql2 .= "                        areacd = '$areacd'," ;
				$sql2 .= "                        grpcd = '$grpcd'," ;
				$sql2 .= "                        qcdate = '$qcdte'," ;
				$sql2 .= "                        qcqty = '$qcqty'," ;
				...
				mysql_query($sql2) or die("Error updating Sew Entry:".mysql_error(). ' Failed SQL is -->'. $sql2);
				//echo $sql2; break;
				//------To delete from sewing barcode before enter new data -------//
				
				$sql = "DELETE FROM sew_barcode where ticketno = '$ticketno'";
				mysql_query($sql) or die("Error Deleting From Sew Barcode :".mysql_error(). ' Failed SQL is -->'. $sql);
				
				//-------------End of delete from sewing barcode ------------------//
				
				
				//---------To get details from job rate (pro_jobmodel) and insert into ticket barcode -----//
				
				$sql = "SELECT prod_jobsec, prod_jobid, prod_jobrate ";
...
//				echo $sql;
				...
				{
				   $prod_jobseq = $jobrate['prod_jobsec'];
			       ...
			       if ($cnt== 0 ){ 				   
				   		...
				   		$sql.= " '$prod_jobrate', '$barcode', '$workid', '$sewdate','$qtydoz','$qtypcs', '','$vartoday','updsewentry')";
			    ...
			    //delete from WIP tran table before insert //
			    $sql = "DELETE FROM wip_tran where rm_receive_id = '$ticketno' and tran_type = 'REC'";
				mysql_query($sql) or die("Error Deleting From WIP Tran table :".mysql_error(). ' Failed SQL is -->'. $sql);
...
<?php
    ...
    $sql = " select  area_desc from area_master ";
    $sql.= " where area_code = '$area'";
    $sql_result = mysql_query($sql);
    $row = mysql_fetch_array($sql_result);
    $area_desc= $row[0];
	  ...
	  <form name="InpJobFMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" onsubmit="return validateForm()">
		...
		<table style="width: 886px">
		  ...
		  <tr>
		   ...
		   <input class="inputtxt" name="prorevdte" id ="prorevdte" type="text" style="width: 128px;" value="<?php echo  $productiondate; ?>" tabindex="4" >
		  ...
		  <tr>
		   ...
		   <input class="inputtxt" name="sexpddte" id ="sexpddteid" type="text" style="width: 128px;" tabindex="4" readonly="readonly" value="<?php  echo $sexpddte; ?>" >
	  	   ...
	  	   <td style="width: 239px; height: 26px;">
		   	<select name="areacd" style="width: 268px">
			    	<?php
			    	    if ($area != ""){
			    			echo '<option value="'.$area.'">'.$area.' | '.$area_desc.'</option>';
			    			echo '<option></option>';
			    		}else{
			    			echo '<option></option>';
			    		}	 
        ...
        echo '>'.$row['grpcd']." | ".$row['grpde'].'</option>';
		 ...
		 <table>
			...
			<?php			  
				$msg = '';
				$var_sqlmsg= " select  count(*) as cntmsg from sew_barcode ";
			    $var_sqlmsg.= " where ticketno = '$ticketno'";
			    $var_sqlmsg.= " and workid is not null";
			    $var_sqlmsg.= " and workid <> '' and workid <> ' ' ";
			    $resultsqlmsg=mysql_query($var_sqlmsg);
				$row = mysql_fetch_array($resultsqlmsg);
				$cntmsg = 0;
				$cntmsg = $row['cntmsg'];
				//echo $cntmsg;
					 ...
					 if ($cntmsg <> 0 ){
				 		...
				 		echo '<span style="color:#FF0000; font-size:xx-large;"><<<--- SEWING ENTRY KEYED IN FOR THIS TICKET NUMBER --->>></span>';
