<?php
    ...
    if($var_loginid == "") { 
      ...
      $wip_adjust_id = $_GET['wip_adjust_id'];
      include("../Setting/ChqAuth.php");
    }
    
    if ($_POST['Submit'] == "Update") {
		$wip_adjust_id = $_POST['wip_adjust_id'];
		...
		if ($refno <> "") {
				...
				$vartoday = date("Y-m-d H:i:s");
				$sql = "update wip_adjust set adjustdate = '$prorevdte', refno ='$refno', remark='$remark', ";
				$sql .= "                       upd_by = '$var_loginid', upd_on='$vartoday' ";
				$sql .= "  where wip_adjust_id = '$wip_adjust_id'";
				mysql_query($sql) or die("Query 1: ".mysql_error());
			
				// to delete from wip adjust details table
				...
				$sql .= "  where rm_receive_id ='$wip_adjust_id' and tran_type = 'OPB'";	
				...
				{	
					foreach($_POST['procomat'] as $row=>$matcd ) {
						...
						if ($matcode <> "")
						{
							...
							$sql = "INSERT INTO wip_adjust_tran values 
						    		('$wip_adjust_id', '$seqno', '$matcode', '$matdesc', '$matuom','$adjustqty','$qtyperpack','$adjustcost','0')";
							mysql_query($sql) or die("Query 4: ".mysql_error());
							
							$sql = "INSERT INTO wip_tran values 
						    		('OPB', '$wip_adjust_id', '$refno','$remark', '$prorevdte', '$matcode', '$adjustcost', '$matdesc', '0', '$adjustqty','$var_loginid', '$vartoday','$var_loginid', '$vartoday')";
							mysql_query($sql) or die("Query 5: ".mysql_error());

           				}	
					}
				}
				$backloc = "../prod/m_wip_adjust.php?menucd=".$var_menucode;
           		echo "<script>";
           		echo 'location.replace("'.$backloc.'")';
            	echo "</script>"; 		
		}else{
			$backloc = "../prod/m_wip_adjust.php?stat=4&menucd=".$var_menucode;
           	echo "<script>";
           	echo 'location.replace("'.$backloc.'")';
            echo "</script>"; 
		}
...
<body onload="setup()">
  ...
  <div class="contentc">
	...
	<fieldset name="Group1" style=" width: 950px;" class="style2">
	 <legend class="title">WIP ADJUSTMENT UPDATE :<?php echo $wip_adjust_id;?></legend>
	  <br>	 
	  ...
	  <form name="InpJobFMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" onsubmit="return validateForm()">
		...
		<table style="width: 886px">
			...
			<input class="textnoentry1" name="wip_adjust_id" id="prorevid" type="text" style="width: 84px;" readonly="readonly" tabindex="0" value="<?php echo $wip_adjust_id; ?>">
			</td>
	  	  </tr> 
	  	  <tr>
	  	   <td></td>
		  ...
		  <table id="itemsTable" class="general-table" style = "width: 900px;">
            ...
            <tbody>
              <?php
             	$sql = "SELECT * FROM wip_adjust_tran";
             	$sql .= " Where wip_adjust_id='".$wip_adjust_id ."'"; 
	    		$sql .= " ORDER BY seqno";  
		 ...
		 <table>
		  	<tr>
				...
				<?php
				 $locatr = "m_wip_adjust.php?menucd=".$var_menucode;
