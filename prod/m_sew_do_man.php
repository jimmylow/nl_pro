<?php
    ...
    if($var_loginid == "") { 
	 ...
	 if ($_POST['Submit'] == "Delete") {
     	if(!empty($_POST['procd']) && is_array($_POST['procd'])) 
     	{
			...
			$sql = " select distinct ticketno from sew_do_man_tran ";
     		$sql.= " WHERE sew_do_manid = '$value'";
    ...
    if ($_GET['p'] == "PrintDO"){
        ...
        $fname = "sew_do_manform.rptdesign&__title=myReport"; 
        ...
        $backloc = "../prod/m_sew_do_man.php?&menucd=".$var_menucode;
       	echo "<script>";
       	echo 'location.replace("'.$backloc.'")';
        echo "</script>"; 
   	}
	...
	if ($_GET['p'] == "PrintGIN"){
        ...
        $fname = "sew_gin_manform1.rptdesign&__title=myReport"; 
        $dest = "http://".$var_prtserver.":8080/birt-viewer/frameset?__report=".$fname."&do=".$dono."&menuc=".$var_menucode."&dbsel=".$varrpturldb;
        $dest .= urlencode(realpath($fname));
...
&nbsp;<form name="LstCatMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode;; ?>">
		 ...
		 <tr>
           ...
           <?php
                $locatr = "sew_do_man.php?menucd=".$var_menucode;
                if ($var_accadd == 0){
         ...
         <thead align="center">
          ...
          <th style="width: 40px">Do Date</th>       
          <th></th>
		  <th></th>
          <th></th>
		  <th></th>
         ...
         <tr>
          ...
          <th class="tabheader" style="width: 40px">DO Date</th>
		 ...
		 <?php 
			...
			while ($rowq = mysql_fetch_assoc($rs_result)) { 
				...
				$sew_do_manid = htmlentities($rowq['sew_do_manid']);
...
//				$qcdate = date('d-m-Y', strtotime($rowq['qcdate']));
				...
				$urlpop = 'upd_sew_do_man.php';
				$urlvie = 'vm_sew_do_man.php';
				echo '<tr bgcolor='.$defaultcolor.'>';
            	echo '<td>'.$numi.'</td>';
           		echo '<td align="left">'.$sew_do_manid.'</td>';
           		echo '<td align="left">'.$buyer_desc.'</td>';
            	echo '<td>'.$dodate.'</td>';
          	
            	
            	// to check the sewing QC, if have record, cannot delete
            	$sql1 = "select count(*) from sew_do_man";
        		$sql1 .= " where sew_do_manid='".$sew_do_manid."' ";
        		$sql1 .= " and posted='Y' ";
				...
				echo '<td align="center"><a href="m_sew_do_man.php?p=PrintDO&donum='.$sew_do_manid.'&menucd='.$var_menucode.'" title="Print Sewing D/O"><img src="../images/b_print.png" border="0" width="16" height="16" hspace="2" alt="Duplicate Sewing D/O" /></a></td>'; 
				echo '<td align="center"><a href="m_sew_do_man.php?p=PrintGIN&donum='.$sew_do_manid.'&menucd='.$var_menucode.'" title="Print GIN"><img src="../images/b_print.png" border="0" width="16" height="16" hspace="2" alt="Duplicate GIN" /></a></td>'; 

				           
            	if ($var_accvie == 0){
            		echo '<td align="center"><a href="#">[VIEW]</a>';'</td>';
            	}else{
            		echo '<td align="center"><a href="'.$urlvie.'?donum='.$sew_do_manid.'&menucd='.$var_menucode.'">[VIEW]</a>';'</td>';
            	}
            	
            	//echo '<td align="center"><a href="m_sew_do_man.php?p=Print&prod_code='.$ticketno.'&menucd='.$var_menucode.'" title="Print Product Costing"><img src="../images/b_print.png" border="0" width="16" height="16" hspace="2" alt="Duplicate Product Costing" /></a></td>'; 
	            ...
	            if ($var_accupd == 0 ){
	            	...
	            	if ($cnt <> 0 ){
		            	...
		            	echo '<td align="center"><a href="'.$urlpop.'?donum='.$sew_do_manid.'&menucd='.$var_menucode.'" title="'.$apstat.'">[EDIT]</a>';'</td>';
	            	}
	            }
           		 echo '</tr>';
            $numi = $numi + 1;
