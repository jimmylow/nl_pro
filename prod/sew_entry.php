			...
			$var_sql = " SELECT count(*) as cnt from sew_entry";
	      	...
	      	if ($res_id->cnt > 0 ){
			   	...
			   	}				foreach ($results as $jobrate) 
				{
				   ...
				   $cnt = 0;
			       $var_sql2 = " SELECT count(*) as cnt from sew_barcode ";
			       $var_sql2 .= " Where barcodeno = '$barcode'";
				   $result=mysql_query($var_sql2);
			       ...
			       if ($cnt== 0 ){ 
				   		...
				   		$sql.= " '$prod_jobrate', '$barcode', '$workid', '$sewdate','$qtydoz','$qtypcs', '', '0000-00-00', 'sew_entry')";
				...
				//$mth = '11'; //user wana back date due to still not finish key in for the previous month				
		        ...
		        //---------------- insert into WIP onhand balance ---------------//
		        
		        $uprice = 0.00;
		        $sql = "select totamt/12 as totamt from prod_matmain ";
				$sql .= " where prod_code = '". $selmaincode. "'";
				...
				if ($qtydoz == '' or $qtydoz == ' ' or $qtydoz == NULL)
				{
					$qtydoz = 0;
				}
				...
				//echo $sql; echo '</br>';
				//echo 'price- '. $uprice; echo '</br>';
				
				
				
				$result = mysql_query("select prod_desc from pro_cd_master where prod_code = '$selmaincode' ")or die ("Cant get desc from  pro_cd_master: " .mysql_error());
				$row = mysql_fetch_row($result);
				$desc = $row[0];
		        ...
		        $sql3 = "INSERT INTO wip_tran values 
				  		('REC', '$ticketno', '$ticketno','$buyerord', '$prorevdte', '$selmaincode', '$uprice', '$desc', '$qtydoz', '$productqty','$var_loginid', '$vartoday','$var_loginid', '$vartoday')";
 ...
 <?php include("../topbarm.php"); ?> 
  ...
  <div class ="contentc">
	...
	<fieldset name="Group1" style="width: 1150px;" class="style2">
	 <legend class="title">SEWING TICKET ENTRY</legend>
	  <?php
/*	  		      				//---------To get details from job rate (pro_jobmodel) and insert into ticket barcode -----//
				
				$sql = "SELECT trxdate, productiondate, ticketno ";
				$sql .=" FROM wip_tran x, sew_entry y ";
				$sql .=" WHERE  x.rm_receive_id = y.ticketno and tran_type = 'REC' ";	
				$sql .=" and productiondate is not null and productiondate > '1980-01-01'";
				//$sql .=" AND x.rm_receive_id = '311B0002' ";
				//echo $sql;
     		...
     		while($row = mysql_fetch_array($dumsysno))
     		{
			   ...
			   $ticketno = $row ['ticketno'];
     		   $trxdate = date("Y-m-d", strtotime($row['trxdate']));
	            //echo "</br>". 'jjjj-'. $productiondate. "</br>"; 
 
		       if ($productiondate<> $trxdate ){ 	
		       		echo 'Updating...';  
			   		$sql2 = " UPDATE wip_tran ";
			   		$sql2.= " SET trxdate = '". $productiondate . "'";
			   		$sql2.= " WHERE tran_type = 'REC' and rm_receive_id = '". $ticketno . "'";
			   		//echo '</br>';
			   		//echo $sql2;
			   		mysql_query($sql2) or die("Error in Sew Ticket Barcode:".mysql_error(). ' Failed SQL is -->'. $sql2);	
			   }else{
			     //echo 'No Updation... ';
			   }
	  ...
	  <form name="InpJobFMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" onsubmit="return validateForm()">
		...
		<table style="width: 886px">
	  	   ...
	  	   <td style="width: 239px">
		   	<select name="sewtypecd" id="sewtypecdid" style="width: 268px" onchange="getzone(this.value)" tabindex="1">
			 <?php
              $sql = "select sewtype_code, sewtype_desc from prosewtype_master ORDER BY sewtype_code ASC ";
              $sql_result = mysql_query($sql);
			  ...
			  if(mysql_num_rows($sql_result)) 
			  {
			   ...
			   { 
				echo '<option value="'.$row['sewtype_code'].'"';;
        //if ($vmcustcd == $row['sew_type_code']) { echo "selected"; }
        echo '>'.$row['sewtype_code']." | ".$row['sewtype_desc'].'</option>';
			   } 
		      } 
		   ...
		   <td></td>
	  	   <td style="width: 126px">Order No. </td>
	  	   <td style="width: 13px">:</td>
	  	   <td style="width: 239px">
		   <input class="autosearch" name="prod_code" id="prod_code" type="text" maxlength="15" style="width: 129px" onblur="getState(this.value)" tabindex="2" >
		   </td>
	  	  </tr>
	  	   <tr>
	  	   <td></td>
		  ...
		  <tr>
	  	   ...
	  	   <td style="width: 13px">:</td>
	  	   <td style="width: 239px">
		   <input name="ticketno" id ="ticketnoid" type="text" style="width: 156px; text-align:center;" tabindex="3" readonly="readonly" ></td>
		   <td></td>
		   <td>Order Date</td>
		   <td>:</td>
		    <td>
		   <input class="inputtxt" name="sorddte" id ="sorddteid" type="text" style="width: 128px;" tabindex="99" readonly="readonly" >
		   </td>
		  </tr> 
		  <tr>
	  	   <td></td>
		  ...
		  <tr>
	  	   ...
	  	   <td style="width: 13px">:</td>
	  	   <td style="width: 239px">
		   <input name="batchno" id ="batchnoid" type="text" style="width: 156px; text-align:center;" tabindex="4" ></td>
		   <td></td>
		   <td>Delivery Date</td>
		   <td>:</td>
		    <td>
		   <input class="inputtxt" name="sexpddte" id ="sexpddteid" type="text" style="width: 128px;" tabindex="99" readonly="readonly" >
		   </td>
	  	  </tr>
	  	   <tr>
	  	   <td></td>
	  	   ...
	  	   <td style="width: 239px; height: 26px;">
		   	<select name="areacd" id="areacdid" style="width: 268px" tabindex="5" >
			 <?php
        ...
        echo '>'.$row['area_code']." | ".$row['area_desc'].'</option>';
		   ...
		   <td>:</td>
		    <td>
		    <input class="inputtxt" name="qcdte" id ="qcdteid" type="text" style="width: 128px;" tabindex="88" >
		   <img alt="Date Selection" src="../images/cal.gif" onclick="javascript:NewCssCal('prorevdte','ddMMyyyy')" style="cursor:pointer"></td>
	  	   ...
	  	   <td style="width: 13px; height: 26px;">:</td>
	  	   <td style="width: 239px; height: 26px;">
		   	<select name="grpcd" id="grpcdid" style="width: 268px" tabindex="6" >
			 <?php
        ...
        echo '>'.$row['grpcd']." | ".$row['grpde'].'</option>';
		   ...
		   <td>:</td>
		    <td>
		   <input class="inputtxt" name="qcqty" id ="qcqtyid" type="text" style="width: 128px;" tabindex="88" >
		   </td>
	  	    </tr>
			<tr>
	  	   <td></td>
		   ...
		   &nbsp;</td>
			...
			<tr>
	  	   ...
	  	   <td style="width: 13px; height: 26px;">:</td>
	  	   <td style="width: 239px; height: 26px;">
		   <input name="sbuycd" id ="sbuycdid" type="text" style="width: 156px; text-align:center;" readonly="readonly" ></td>
		   <td></td>
		   <td style="width: 136px">&nbsp;</td>
		   <td>&nbsp;</td>
		   <td>
