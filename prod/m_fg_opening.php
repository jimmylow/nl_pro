<?php
	 ...
	 if ($_POST['Submit'] == "Delete") {
     	if(!empty($_POST['procd']) && is_array($_POST['procd'])) 
     	{
           foreach($_POST['procd'] as $key) {
		     ...
		     $sql = "DELETE FROM fg_opening "; 
		     $sql .= "WHERE fg_opening_id ='".$var_proccd."'";  
		 	 mysql_query($sql); 
		   ...
		   $backloc = "../prod/m_fg_opening.php?stat=1&menucd=".$var_menucode;
           echo "<script>";
	...
	if (isset($_POST['Submit'])){ 
     if ($_POST['Submit'] == "Print") {
     	 ...
     	 $backloc = "../stck_tran/inq_fg_open.php?menucd=".$var_menucode;
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>";   
     }
...
<body>
  ...
  <div class="auto-style1">
	...
	<fieldset name="Group1" style=" width: 800px;" class="style2">
	 <legend class="title">Finish Goods OPENING LISTING</legend>
	  <br>
        ...
        <form name="LstCatMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode;; ?>">
		 ...
		 <tr>
           ...
           <?php
                $locatr = "fg_opening.php?menucd=".$var_menucode;
                if ($var_accadd == 0){
				...
				//}
    	  	   ...
    	  	   $msgdel = "Are You Sure Delete FG opening List?";
		 ...
		 <tbody>
		 <?php 
		    $sql = "SELECT distinct fg_opening_id, refno, openingdate, upd_by, upd_on ";
		    $sql .= " FROM fg_opening ";
    		$sql .= " ORDER BY fg_opening_id";   
			...
			while ($rowq = mysql_fetch_assoc($rs_result)) { 
				...
				$fg_opening_id = htmlentities($rowq['fg_opening_id']);
				...
				$urlpop = 'upd_fg_opening.php';
				$urlvie = 'vm_fg_opening.php';
				echo '<tr bgcolor='.$defaultcolor.'>';
				
            	echo '<td>'.$numi.'</td>';
           		echo '<td>'.$fg_opening_id.'</td>';
           		echo '<td>'.$refno.'</td>';
            	...
            	if ($var_accvie == 0){
            		...
            		echo '<td align="center"><a href="'.$urlvie.'?fg_opening_id='.$fg_opening_id.'&menucd='.$var_menucode.'">[VIEW]</a>';'</td>';
            	}
	            if ($var_accupd == 0){
		            echo '<td align="center"><a href="#">[EDIT]</a>';'</td>';
	            }else{
		            echo '<td align="center"><a href="'.$urlpop.'?fg_opening_id='.$fg_opening_id.'&menucd='.$var_menucode.'">[EDIT]</a>';'</td>';
	            }
	            if ($var_accdel == 0){
