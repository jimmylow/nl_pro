<?php
    ...
    if ($_POST['Submit'] == "Update") {
		...
		if ($refno <> "") {
					...
					foreach($_POST['procomat'] as $row=>$matcd ) {
						...
						if ($matcode <> "")
						{
							...
							$sql = "INSERT INTO wip_opening_tran values 
						    		('$wip_opening_id', '$seqno', '$matcode', '$matdesc', '$matuom','$openingqty','$qtyperpack','$openingcost','0')";
							mysql_query($sql) or die("Query 4: ".mysql_error());
							
							$sql = "INSERT INTO wip_tran values 
						    		('OPB', '$wip_opening_id', '$refno','$remark', '$prorevdte', '$matcode', '$openingcost', '$matdesc', '0', '$openingqty','$var_loginid', '$vartoday','$var_loginid', '$vartoday')";
			...
			$backloc = "../prod/m_wip_opening.php?stat=4&menucd=".$var_menucode;
           	echo "<script>";
           	echo 'location.replace("'.$backloc.'")';
            echo "</script>"; 
		}
...
<body onload="setup()">
  ...
  <div class="contentc">
	...
	<fieldset name="Group1" style=" width: 950px;" class="style2">
	 <legend class="title">WIP OPENING UPDATE :<?php echo $wip_opening_id;?></legend>
	  <br>	 
