<?php
    ...
    if (isset($_POST['Submit'])){ 
     if ($_POST['Submit'] == "Print") {
		...
		$sql  = "select prod_code, ticketno, prod_jobid, prod_jobrate";
		$sql .= " from sew_barcode";
		$sql .= " where workid = ''";
		$sql .= " and 	prod_code between '$fpr' and '$tpr'";
		$sql .= " and   ticketno in (select ticketno from sew_entry where qcqty > 0";
        $sql .= "                    and productiondate between '$fdte' and '$tdte'";
        $sql .= "                    and stat = 'ACTIVE')";		
		...
		while ($row = mysql_fetch_assoc($rs_result)) { 
		   	...
		   	if (empty($prod_jobrate)){$prod_jobrate = 0;}
		   	
	   		$sqlc  = "select productqty, productiondate";
	    	$sqlc .= " from sew_entry";
       		$sqlc .= " where ticketno ='$ticketno' and productcode = '$prod_code'";
       		$sql_resultc = mysql_query($sqlc);
       		$rowc = mysql_fetch_array($sql_resultc);
	   		$productqty = $rowc['productqty'];
	   		$productiondate = $rowc['productiondate'];
		   	if (empty($productqty)){$productqty = 0;}
			if (empty($productiondate)){$productiondate = '2000-01-01';}
			...
			$bal = $productqty;
			if (empty($bal)){$bal = 0;}
			$wage = $bal * $prod_jobrate / 12;
		   	       			    	
		    $sqliq  = " Insert Into tmpiwipqc";
        	$sqliq .= " Values ('$var_loginid', '$prod_code', '$productiondate', '$ticketno', '$productqty', '$prod_jobid',";
        	$sqliq .= "         '$jobfile_desc', '$prod_jobrate', '$bal', '$wage')";
			//echo $sqliq."<br>";
        	mysql_query($sqliq) or die("Unable Save In Temp Table ".mysql_error());
		 }

     	 $fname = "iwip.rptdesign&__title=myReport"; 
         ...
         $backloc = "../prod_rpt/iwip.php?menucd=".$var_menucode;
       	 echo "<script>";
