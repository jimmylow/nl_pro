    ...
    if (isset($_POST['Submit'])){ 
     if ($_POST['Submit'] == "Print") {
		...
		while ($row = mysql_fetch_assoc($rs_result)) { 
		    ...
		    #------------Supplier Name-----------------------------			
			$sqlop = "select supplier_desc from supplier_master ";
        	...
        	#----------------local currency---------------------------------------------
        	$sqld  = " select y.currency_code";
            $sqld .= "  from po_trans x, rawmat_price_ctrl y, po_master z";
            $sqld .= "  where x.itemcode = y.rm_code ";
		    $sqld .= "  and x.po_no = z.po_no";
            $sqld .= "  and z.supplier = y.supplier";
            $sqld .= "  and x.po_no = '$pono'";
            $sqld .= "  and y.rm_code = '$mitmcd'";
            $sql_resultd = mysql_query($sqld);
        	$rowd = mysql_fetch_array($sql_resultd);
        	$currcy = $rowd[0];
        	if ($currcy == "MYR"){
        	...
        	$sqliq  = " Insert Into tmppobal (ponum, suppcd, suppde, itemcd, ";
        	$sqliq .= "  itemde, ordqty, recqty, deldte, orddte, lstrecdte, usernm, poprice, ";
        	$sqliq .= "  compflg, itemuom, avgcst, rmpoprice, curry)";
        	$sqliq .= " Values ('$pono', '$supp', '$suppde', '$mitmcd', '$itmdesc',";
        	$sqliq .= "   '$qtyord', '$recbal', '$deldte', '$podte', '$lstgdte', '$var_loginid',";
        	$sqliq .= "   '$poprice', '$compflg', '$uom', '$avgcst', '$rmpopri', '$currcy')";
			switch ($compflg){
				case 'A':
		...
		if($cnt == 0){
			...
			// Redirect browser
        	$fname = "pur_bal.rptdesign&__title=myReport"; 
        	$dest = "http://".$var_prtserver.":8080/birt-viewer/frameset?__report=".$fname."&fd=".$fpodte."&td=".$tpodte."&usernm=".$var_loginid."&s=".$compflg."&dbsel=".$varrpturldb;
