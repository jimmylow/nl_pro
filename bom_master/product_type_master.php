<?php
    ...
    if ($_POST['Submit'] == "Save") {
     ...
     if ($typecd <> "") {
      ...
      if ($res_id->cnt > 0 ) {
	     $backloc = "../bom_master/product_type_master.php?stat=3&menucd=".$var_menucode;
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>";  
      }else {
         $sql = "INSERT INTO protype_master values 
          ('$typecd','$typedesc', '$var_loginid', CURDATE(), '$var_loginid', CURDATE(), 'A')";
echo $sql;
     	 mysql_query($sql) or die("Unable to Insert Type ".mysql_error()); 
     	 $backloc = "../bom_master/product_type_master.php?stat=1&menucd=".$var_menucode;
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>";
      } 
     }else{
         $backloc = "../bom_master/product_type_master.php?stat=4&menucd=".$var_menucode;
         echo "<script>";
         echo 'location.replace("'.$backloc.'")';
         echo "</script>";     }
    }
    ...
    if ($_POST['Submit'] == "Delete") {
     if(!empty($_POST['typecd']) && is_array($_POST['typecd'])) 
     {
           ...
           foreach($_POST['typecd'] as $value ) {
		    $sql = "DELETE FROM protype_master WHERE type_code ='".mysql_real_escape_string($value)."'"; 
		 	mysql_query($sql); 
		   }
		   $backloc = "../bom_master/product_type_master.php?stat=1&menucd=".$var_menucode;
           echo "<script>";
    ...
    if (isset($_POST['Submit'])){ 
     if ($_POST['Submit'] == "Print") {
        ...
        $fname = "protype_rpt.rptdesign&__title=myReport"; 
...
<?php include("../topbarm.php"); ?> 
  <div class="contentc">
	...
	<fieldset name="Group1" style=" width: 714px;" class="style2">
	 <legend class="title">PRODUCT TYPE MASTER</legend>
	  <br>
	  ...
	  <form name="InpTypeMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" style="height: 134px; width: 696px;">
		<table>
	  	  <tr>
	  	    ...
	  	    <td style="width: 138px" class="tdlabel">Product Type&nbsp;Code</td>
	  	    <td>:</td>
	  	    <td style="width: 505px">
			<input class="inputtxt" name="typecd" id ="typecdid" type="text" maxlength="10" onchange ="upperCase(this.id)" onBlur="AjaxFunction(this.value);" style="width: 61px">
			</td>
	  	  </tr>
	  	  <tr>
	  	    <td></td> 
        ...
        <form name="LstTypeMas" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>">
         ...
         <thead>
         	...
         	<tr>
         	 ...
         	 <th class="tabheader" style="width: 138px">Product Type Code</th>
		 ...
		 <tbody>
		 <?php 
				$sql = "SELECT type_code,  type_desc, stat ";
				$sql .= " FROM protype_master";
    		    $sql .= " ORDER BY stat, type_code";  
            ...
            echo '<td align="center"><a href="'.$urlpop.'?typecd='.htmlentities($row['type_code']).'&typede='.htmlentities($row['type_desc']).'&menucd='.$var_menucode.'">[EDIT]</a>';'</td>';
            }
            if ($var_accdel == 0){
              echo '<td align="center"><input type="checkbox" DISABLED  name="typecd[]" value="'.$rowq['type_code'].'" />'.'</td>';
            }else{
              echo '<td align="center"><input type="checkbox" name="typecd[]" value="'.htmlentities($row['type_code']).'" />'.'</td>';
            }
            echo '</tr>';
            $numi = $numi + 1;
			}
