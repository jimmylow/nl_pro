<?php
    ...
    if($var_loginid == "") { 
      ...
      $srmk2     = $row['rmk2'];
      $srmk3     = $row['rmk3'];
      $banknm   = htmlentities($row['sbknm']);
	  $bankadd1 = htmlentities($row['sbkadd1']);
	  $bankadd2 = htmlentities($row['sbkadd2']);
	  $bankadd3 = htmlentities($row['sbkadd3']);
	  $sbanktel = htmlentities($row['sbktel']);
	  $sbankfax = htmlentities($row['sbkfax']);
	  $sbankacc = htmlentities($row['sbkaccno']);
	  $sbankswi = htmlentities($row['sbkswicd']);
	  $fabcont  = $row['fabcont'];
	                         	
	  $sql  = "INSERT INTO tmpinvshipremark (usernm, banknm, bankadd1, bankadd2, bankadd3, tel, fax, accno, swiftno) ";
	  $sql .= " values ('$var_loginid', '$banknm', '$bankadd1', '$bankadd2', '$bankadd3', '$sbanktel', '$sbankfax', ";
    ...
    if ($_POST['Submit'] == "Save") {
        $sinvno    = mysql_real_escape_string($_POST['sinvno']);
		...
		if ($sbuycd <> "") {
            ...
            $vartoday = date("Y-m-d");
			$sql  = "update ship_invmas set invdte = '$sinvdte', buycd = '$sbuycd', etddte = '$setddte', ";
			$sql .= "                       etadte = '$setadte', buyorderno = '$sordno', vesdet = '$svesdet', ";
			$sql .= "                  		couori = '$scountori', salcondi = '$scondi', taxper = '$gstper', ";
			$sql .= "                       buynm = '$sbuyerde', buytel = '$sbuytel', rmk1 = '$srmk1', rmk2 = '$srmk2', ";
			$sql .= "                       rmk3 ='$srmk3',	lastupdat = '$vartoday', lastupdby = '$var_loginid', ";
			$sql .= "                       sbknm = '$rbanknm', sbkadd1 = '$rbankadd1', sbkadd2 = '$rbankadd2', ";
			$sql .= "                       sbkadd3 = '$rbankadd3', sbktel = '$rtel', sbkfax = '$rfax',";
			$sql .= "                       sbkaccno = '$raccno', sbkswicd = '$rswiftno', fabcont = '$fabcont'";
			$sql .= " where invno = '$sinvno'";
			mysql_query($sql) or die("Error insert Sales Entry:".mysql_error(). ' Failed SQL is --> '. $sql);

			$sql = "delete from ship_invdet where invno = '$sinvno'";
					...
					foreach($_POST['procd'] as $row=>$procd ) {
						...
						if ($procode <> "" or $procdbuyer <> "")
						{
							$sql = "INSERT INTO ship_invdet values 
						    		('$sinvno', '$procode', '$procdbuyer', '$prodesc', '$proqty', '$prouom', '$proupri', '$protamt', '$proseqno')";
  ...
  <div class="contentc">
	...
	<fieldset name="Group1" style=" width: 1020px;" class="style2">
	 <legend class="title">SHIPPING INVOICE</legend>
	  ...
	  <form name="InpSalesF" method="POST" action="<?php echo $_SERVER['PHP_SELF'].'?menucd='.$var_menucode; ?>" onsubmit="return validateForm()" style="width: 1000px">
		...
		<table style="width: 993px; height: 220px;">
			...
			<input class="inputtxt" name="sinvno" id="sinvno" type="text"  style="width: 160px;" readonly="readonly" value="<?php echo $sinvd; ?>">
			</td>
		  ...
		  <tr>
		   ...
		   <td style="width: 284px;">
			...
			<tr>
	  	   ...
	  	   <td style="width: 13px;">:</td>
	  	   <td style="width: 201px;">
			<input class="inputtxt" name="sbuytel" id="sbuytel" type="text" maxlength="60" style="width: 263px;" value="<?php echo $sbuytel; ?>"></td>
		   <td style="width: 10px;"></td>
